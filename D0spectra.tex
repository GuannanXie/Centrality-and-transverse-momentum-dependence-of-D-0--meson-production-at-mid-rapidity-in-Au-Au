...
Events are selected with the offline reconstructed collision vertex ~\cite{Smirnov_2017} within 6 cm of the TPC and HFT centers along the beam direction to ensure uniform and large acceptance. Event pileup due to the long TPC readout time ($\sim$\,40\,$\upmu$s) with respect to the Au-Au collision rate (40 KHz) is rejected by removing events that have an online and offline vertex position difference along the z-axis greater than 3 cm. Approximately 9$\times 10^{8}$ minimum bias triggered events with 0--80\% centrality pass the selection criteria and are used in this analysis.
